var AltoView = Class.create({
		...
		textBlock.lines.each(function(textLine) {
			...
			textLine.strings.each(function(word) {
				if(!word["delete"]) {
					...
					var altColor = null, bgColor = null;
					if(!word.updated && (
						word.content.match(/([cbdfghjklmnpqrstvwxz])f([cbdfghjkmnpqtvwx])/) || 
						word.content.match(/^f([cbdfghjkmnpqtvwx])/) ||
						word.content.match(/fch/)
					)) {
						word.content = word.content.replace(/([cbdfghjklmnpqrstvwxz])f([cbdfghjkmnpqtvwx])/g, '$1s$2');
						word.content = word.content.replace(/^f([cbdfghjkmnpqtvwx])/g, 's$1');
						word.content = word.content.replace(/fch/g, 'sch');
						_this.insertUpdateField("update", span.id, "content", word.content);
						word.updated = true;
						altColor = "#0044dd";
						bgColor = "#dddd00";
					}
					...
					if(word.updated) {
						span.style.color = (altColor ? altColor : "green");
						span.style.backgroundColor = (bgColor ? bgColor : "transparent");
					...
					span.observe("mouseout", function(e) {
						this.style.backgroundColor = (bgColor ? bgColor : "transparent");
					});
				}
			});
			lineDiv.observe("click", function(e) {
				...
				if(!this.childElements().detect(function(e) { return e.id.match(/_input$/); }) && !_this.lockLineClick)
					_this.editWord(this.childElements()[1]);
			});
		...
		insertions.each(function(insert) {
			...
			for(var i = 0; i < spans.length; ++i) {
				...
				if((word && nextWord && word.hpos < insert.hpos && nextWord.hpos > insert.hpos) || (word && !nextWord && word.hpos < insert.hpos)) {
					var newSpan = new Element("span", {"id": lineDiv.ancestors()[0].id + ":" + lineDiv.id + ":" + insert.id, "style": "color: green; font-weight: bold"});
	...
	saveCurrentUpdate: function() {
		if(this.current_edit) {
			...
			if(this.current_edit.input.value != span.innerHTML) {
				...
				span.stopObserving("mouseout");
				span.observe("mouseout", function(e) {
					this.style.backgroundColor = "transparent";
				});
			} else if(span.style.color == "#0044dd") {
				span.style.color = "green";
				span.style.backgroundColor = "transparent";
			}
			...
			input.observe("keydown", function(e) {
				...
				if(k == 9) {
					...
					if(document.selection) {
						...
						selLength = this.selectionEnd - this.selectionStart;
					}
					if(caretPos == 0 && k == 37 && selLength < this.value.length) {
	...
	findAndReplace: function(fnd, repl) {
		this.saveCurrentUpdate();
		if(fnd.match(/\s/) || fnd == "") {
			alert("Zoekterm mag niet leeg zijn en geen spaties bevatten");
			return false;
		} else if(repl.match(/\s/) || repl == "") {
			alert("Vervangingsterm mag niet leeg zijn en geen spaties bevatten");
			return false;
		}

		var _this = this;
		var cnt = 0;
		$$(".altoLineDiv span").each(function(span) {
			if(span.innerHTML == fnd) {
				span.innerHTML = span.innerHTML.replace(fnd, repl);
				_this.insertUpdateField("update", span.id, "content", repl);
				span.style.color = "green";
				span.style.fontWeight = "bold";
				cnt++;
			}
		});
		alert(cnt + " wijziging" + (cnt == 1 ? "" : "en") + " aangebracht");
	}
});
